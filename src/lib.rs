//! Internal implementation of the `vdt` command-line tool.
//!
//! This crate is not a supported Rust library. Its items are public only so
//! the `vdt` binary, the WebAssembly bindings in `bindings/wasm`, and the
//! integration tests can share one converter. They are hidden from generated
//! docs and may change in any release without notice; use the `vdt` CLI.
