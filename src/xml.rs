...
use crate::vector::{FillRule, LineCap, LineJoin, PathCommand, VectorDrawable};
...
pub fn write(drawable: &VectorDrawable) -> String {
    ...
    for path in &drawable.children {
        ...
        if let Some(color) = path.stroke {
            let _ = writeln!(
                xml,
                "        android:strokeColor=\"#{:02X}{:02X}{:02X}\"",
                color.0, color.1, color.2
            );
            let _ = writeln!(
                xml,
            ...
            if path.stroke_cap != LineCap::Butt {
                ...
                let _ = writeln!(xml, "        android:strokeLineCap=\"{value}\"");
            }
            if path.stroke_join != LineJoin::Miter {
                ...
                let _ = writeln!(xml, "        android:strokeLineJoin=\"{value}\"");
            }
