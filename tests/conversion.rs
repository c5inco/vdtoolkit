...
fn lowers_elliptical_radial_gradients_via_group_transform() {
    ...
    assert_eq!(asset.analysis.compatibility, Compatibility::ExactWithNormalization);
    let xml = asset.to_xml();
    assert!(xml.contains("<group"), "XML should contain group: {xml}");
    assert!(xml.contains("android:scaleY="), "Group should scale Y: {xml}");
    assert!(xml.contains("android:pivotX="), "Group should specify pivot: {xml}");
    roxmltree::Document::parse(&xml).unwrap();
    ...
    assert_eq!(asset.analysis.compatibility, Compatibility::ExactWithNormalization);
    let xml = asset.to_xml();
    assert!(xml.contains("android:rotation="), "Group should specify rotation: {xml}");
    roxmltree::Document::parse(&xml).unwrap();
}
...
fn radial_gradient_focal_offset_converts_with_allow_approximate() {
    ...
    assert!(default_analysis.diagnostics.iter().any(|d| d.code == DiagnosticCode::UnsupportedGradient));
    ...
    assert!(approx_analysis.diagnostics.iter().any(|d| d.code == DiagnosticCode::UnsupportedGradient && matches!(d.severity, Severity::Warning)));
...
fn elliptical_radial_fill_with_stroke_splits_into_group_and_path() {
    ...
    assert!(xml.contains("android:strokeColor=\"#FF0000\""), "Should have stroke path: {xml}");
    assert!(xml.contains("android:strokeWidth=\"2\""), "Stroke width should be unscaled: {xml}");
}
