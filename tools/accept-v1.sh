...
"$install_root/bin/svg2vd" --version

version=$(target/release/svg2vd --version | awk '{print $2}')
target_name=$(rustc -vV | awk '/^host:/ {print $2}')
package="svg2vd-v${version}-${target_name}"
destination="target/release-dist"
rm -rf "$destination"
mkdir -p "$destination/$package"
cp target/release/svg2vd README.md LICENSE-MIT LICENSE-APACHE "$destination/$package/"
...
"$package_root/$package/svg2vd" --version

if command -v ldd >/dev/null 2>&1; then
  if ldd target/release/svg2vd | grep -Eiq 'android|java|jvm'; then
