...
# Isolate spelling from rounding for issue #25. Take the glow's absolute
# controls from convert, round them independently, and retain every non-path
# attribute from optimize. Do not decode the optimized relative path here:
# doing so would copy any relative-coordinate drift into the control.
python3 - "$BUILD/generated/res/drawable-v24" <<'PY'
import pathlib
import re
import sys
...
converted = ET.parse(directory / "elliptical_glow.xml")
paths = converted.findall(".//path")
assert len(paths) == 1
data = paths[0].attrib["{http://schemas.android.com/apk/res/android}pathData"]
assert set(re.findall(r"[A-Za-z]", data)) <= set("MLCZ"), data
rounded = re.sub(
    r"-?\d+(?:\.\d+)?",
    lambda match: str(Decimal(match[0]).quantize(Decimal("0.001"), rounding=ROUND_HALF_UP)),
    data,
)
optimized = (directory / "elliptical_glow_optimized.xml").read_text()
absolute, count = re.subn(r'android:pathData="[^"]*"', f'android:pathData="{rounded}"', optimized)
assert count == 1
(directory / "elliptical_glow_rounded_absolute.xml").write_text(absolute)
PY

# One pinned Material symbol is rendered side-by-side with its source SVG in a
# WebView. The competing XMLs are pre-generated by their respective benchmark
# tools so Android can render all three VectorDrawables from the same source.
cp "$HARNESS/fixtures/fidelity/vdt.xml" "$BUILD/generated/res/drawable/fidelity_vdt.xml"
cp "$HARNESS/fixtures/fidelity/ashung.xml" "$BUILD/generated/res/drawable/fidelity_ashung.xml"
cp "$HARNESS/fixtures/fidelity/aosp.xml" "$BUILD/generated/res/drawable/fidelity_aosp.xml"

# Set these three directories to package a corpus as 25-icon pages. Each page
# can be shown on one Android surface (WebView source, vdtoolkit, Ashung, or
# AOSP) and captured with a single display screenshot.
if [[ -n "${FIDELITY_CORPUS:-}" ]]; then
  : "${FIDELITY_ASHUNG_OUTPUTS:?set FIDELITY_ASHUNG_OUTPUTS with matching XML}"
  : "${FIDELITY_AOSP_OUTPUTS:?set FIDELITY_AOSP_OUTPUTS with matching XML}"
  mkdir -p "$BUILD/assets/fidelity"
  index=0
  while IFS= read -r fixture; do
    filename=$(basename "$fixture" .svg)
    printf -v number '%03d' "$index"
    cp "$fixture" "$BUILD/assets/fidelity/$number.svg"
    "$ROOT/target/debug/vdt" convert "$fixture" \
      --output "$BUILD/generated/res/drawable/fidelity_vdt_$number.xml"
    cp "$FIDELITY_ASHUNG_OUTPUTS/$filename.xml" \
      "$BUILD/generated/res/drawable/fidelity_ashung_$number.xml"
    cp "$FIDELITY_AOSP_OUTPUTS/$filename.xml" \
      "$BUILD/generated/res/drawable/fidelity_aosp_$number.xml"
    index=$((index + 1))
  done < <(find "$FIDELITY_CORPUS" -maxdepth 1 -type f -name '*.svg' -print | sort)
  echo "Packaged $index fidelity fixtures in $(( (index + 24) / 25 )) pages"
fi
...
# anydpi-v24 vector plus lossless WebPs for API 21 to 23.
"$ROOT/target/debug/vdt" adaptive \
  --foreground "$BUILD/adaptive-src/$(basename "$ADAPTIVE_FOREGROUND")" \
...
done < <(find "$BUILD/generated/res" -type f \( -name '*.xml' -o -name '*.webp' \) -print0)
...
"$BUILD_TOOLS/aapt2" link -o "$BUILD/app-unsigned.apk" \
  --manifest "$HARNESS/app/AndroidManifest.xml" -A "$HARNESS/app/assets" -A "$BUILD/assets" -I "$ANDROID_JAR" \
